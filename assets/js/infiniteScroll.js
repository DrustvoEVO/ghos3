...
Importantly, for this script to work, it requires that each card have the `post` class and that the card container have the `ghos3-postfeed` class
*/
                ...
                if (entry.isIntersecting) {
                    ...
                    if (link) {
                        getNextPage(link).then(({posts, nextLink}) => {
                            posts.forEach(post => {
                                document.querySelector('.ghos3-postfeed').append(post)
                            })
